...
class DabMusicService implements SearchService, MusicDownloader {
    ...
    async download(trackId: number, name: string, roomId: string): Promise<string> {
        const {url: streamingUrl} = await fetch("https://dab.yeet.su/api/stream?trackId=" + trackId, {
            "credentials": "include",
            "headers": {
                "User-Agent": "Mozilla/5.0 (X11; Linux x86_64; rv:137.0) Gecko/20100101 Firefox/137.0",
                "Accept": "*/*",
